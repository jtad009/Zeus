...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Student Type'), ['action' => 'edit', $studentType->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Student Type'), ['action' => 'delete', $studentType->id], ['confirm' => __('Are you sure you want to delete # {0}?', $studentType->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Student Types'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Student Type'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Students'), ['controller' => 'Students', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Student'), ['controller' => 'Students', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="studentTypes view large-9 medium-8 columns content">
    <h3><?= h($studentType->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= h($studentType->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($studentType->created) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Type') ?></h4>
        <?= $this->Text->autoParagraph(h($studentType->type)); ?>
    </div>
    <div class="related">
        <h4><?= __('Related Students') ?></h4>
        <?php if (!empty($studentType->students)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($studentType->students as $students): ?>
            <tr>
                <td><?= h($students->id) ?></td>
