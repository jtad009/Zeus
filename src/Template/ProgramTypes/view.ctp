...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Program Type'), ['action' => 'edit', $programType->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Program Type'), ['action' => 'delete', $programType->id], ['confirm' => __('Are you sure you want to delete # {0}?', $programType->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Program Types'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Program Type'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Students'), ['controller' => 'Students', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Student'), ['controller' => 'Students', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="programTypes view large-9 medium-8 columns content">
    <h3><?= h($programType->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= h($programType->id) ?></td>
        </tr>
        <tr>
    ...
    <div class="row">
        <h4><?= __('Type') ?></h4>
        <?= $this->Text->autoParagraph(h($programType->type)); ?>
    </div>
    <div class="related">
        <h4><?= __('Related Students') ?></h4>
        <?php if (!empty($programType->students)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($programType->students as $students): ?>
            <tr>
                <td><?= h($students->id) ?></td>
